...
def plot_sales_order_target(df_fig, target_year, target_month):
    ...
    ax2.yaxis.set_major_formatter(PercentFormatter(xmax=1))  # Format target percentage

    # Annotate totals on bars (Sales on the left, Orders on the right)
    for i in range(len(df_fig)):
        total_sales = df_fig['sales_quantity'][i] + df_fig['sales_quantity_timber'][i]
        total_orders = df_fig['order_quantity'][i] + df_fig['order_quantity_timber'][i]

        # Sales Total (Left)
        ax1.annotate(f'{total_sales:,.0f}', 
                     (x[i] - width/2, total_sales),
                     textcoords="offset points", 
                     xytext=(-15, 0),  # Shift left
                     ha='right',
                     va='top',
                     fontsize=12,
                     fontweight='bold',
                     color='#006400')  # Green for sales

        # Order Total (Right)
        ax1.annotate(f'{total_orders:,.0f}', 
                     (x[i] + width/2, total_orders),
                     textcoords="offset points", 
